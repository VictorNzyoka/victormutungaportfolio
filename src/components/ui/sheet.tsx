...
function SheetPortal({ className, ...props }: DialogPrimitive.DialogPortalProps) {
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed z-50 grid gap-0 bg-neutral-950 p-0 text-neutral-200 shadow-xl outline-none transition",
...
const SheetHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("border-b border-neutral-900/60 px-4 py-3", className)} {...props} />
)
